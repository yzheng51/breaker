...
# 让机器自动分配device，有GPU会自动放到gpu中
config.allow_soft_placement = True
# 打印算子分配到device的记录日志
# 但是ipython中貌似没有正常输出，可以通过terminal启动python，然后执行同样代码确认是否用上GPU
...
class Breaker(object):
    def __init__(self, embedding_dim, learning_rate,
                target_cycle=1000, verbose=1, lamda=1e-6, alpha=1,earlystop_patience=1,
                 encode_dims=[256, 128, 64], tower_layers=[32, 10], n_cluster=6, cluster_alpha=1
                 , random_seed=2023):
                ...
                all_weights['feature_embeddings_{}'.format(column)] = tf.get_variable(
                    initializer=tf.random_normal(
                        ...
                        shape=[
                            ...
                            # int(self.category_size[column].values[0]) + 2,
                            self.embedding_dim],
                        mean=0.0,
                        stddev=0.01),
    ...
    def metrics(self, true_pred):
        logger.info('apply: %d, %d, %d, %d' % (sum(true_pred['apply_true']), sum(true_pred['apply_pred']), len(
            true_pred['apply_true']), len(true_pred['apply_pred'])))

        is_save = []

        auc_apply = roc_auc_score(y_true=true_pred['apply_true'], y_score=true_pred['apply_pred'])
        logloss_apply = log_loss(y_true=true_pred['apply_true'], y_pred=true_pred['apply_pred'])
        logger.info('auc_apply:%.10f' % (auc_apply))
        if auc_apply >= self.best_auc:
            self.best_auc = auc_apply
            is_save.append(True)
        else:
            is_save.append(False)
        if logloss_apply <= self.best_loss:
            self.best_loss = logloss_apply
            is_save.append(True)
        else:
            is_save.append(False)
...
if __name__ == "__main__":
    ...
    candidate_item = [25344,20641,24807,25355,24814,22383,25361,25270,26488,26489,26490,25343]
